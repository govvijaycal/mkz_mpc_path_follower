...
function interpolate_path(target_vel::Float64, msg::Path, t_ref::Array{Float64,1})
    ...
    t_arr = []            # estimated time the vehicle will reach waypoint i.
    x_arr = []            # planned x coordinate wrt vehicle local coordinate system (base_link)
    y_arr = []            # planned y coordinate wrt vehicle local coordinate system (base_link)
    psi_arr = []          # planned heading wrt vehicle local x-axis (base_link)
    ...
    x_ref = []            # computed target x coordinate wrt vehicle local coordinate system (base_link) at time given by t_ref        
    y_ref = []            # computed target y coordinate wrt vehicle local coordinate system (base_link) at time given by  t_ref        
    psi_ref = []          # computed target heading wrt vehicle local coordinate system (base_link) at time given by  t_ref        
...
function path_callback(msg::Path)
    current_msg = msg
    interpolate_path(target_vel, current_msg, t_ref)
end
